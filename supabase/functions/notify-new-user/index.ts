...
Deno.serve(async (req) => {
    try {
        ...
        const record = payload.record; // new row from profiles table
        ...
        // Get all admins who want user notifications
...
<b>Имя:</b> ${record.full_name || 'Не указано'}
<b>Email:</b> ${record.email || '—'}
<b>ID:</b> <code>${record.id}</code>

Новый пользователь зарегистрировался на сайте.`.trim();
